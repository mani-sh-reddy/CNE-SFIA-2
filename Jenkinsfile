        ...
        stages{
            ...
            stage('Run Pytest Script in Test VM') {
                 steps{
                    ...
                    docker run -e DB_USERNAME=$DB_USERNAME -e DB_PASSWORD=$DB_PASSWORD -e DB_ENDPOINT_TEST=$DB_ENDPOINT_TEST -e DB_NAME_TEST=$DB_NAME_TEST -e SECRET_KEY=$SECRET_KEY -d -p 5000:5000 --name frontend frontend
                    
                    docker run -e DB_USERNAME=$DB_USERNAME -e DB_PASSWORD=$DB_PASSWORD -e DB_ENDPOINT_TEST=$DB_ENDPOINT_TEST -e DB_NAME_TEST=$DB_NAME_TEST -e SECRET_KEY=$SECRET_KEY -d -p 5001:5001 --name backend backend
